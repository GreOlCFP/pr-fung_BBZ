...
def generate_pdf(data):
    ...
    for date_obj in sorted(data["Date_obj"].dropna().unique()):

        group = data[data["Date_obj"] == date_obj]
        ...
        c.drawString(50, y, pd.to_datetime(date_obj).strftime("%d.%m.%Y"))
        y -= 20
...
if filtered_df.empty:
    ...
    for date_obj in sorted(filtered_df["Date_obj"].dropna().unique()):

        group = filtered_df[filtered_df["Date_obj"] == date_obj]
